...
class RS(size: Int = 2, pipelined: Boolean = true, fifo: Boolean = false, priority: Boolean = false, checkpoint: Boolean = false, storeBarrier: Boolean = false, storeSeq: Boolean = false, name: String = "unnamedRS") extends NutCoreModule with HasRSConst with HasBackendConst {
  ...
  if (!pipelined) {
    ...
    when((io.flush || needMispredictionRecovery(brMaskPReg)) && fuValidReg || (io.flush || needMispredictionRecovery(brMask(dequeueSelect))) && io.out.fire()) {
      fuFlushReg := true.B
    }
    when(io.commit.get) {
      fuFlushReg := false.B
    ...
    when(fuValidReg) {
      io.out.bits := fuDecodeReg
    }
    when(fuValidReg) {
      io.out.valid := true.B && !fuFlushReg
    }
  ...
  if (fifo) {
    ...
    when(io.flush) {
      (0 until rsSize).map(i => priorityMask(i) := VecInit(Seq.fill(rsSize)(false.B)))
    }
  }
  ...
  if (checkpoint) {
    ...
    when(io.flush) {
      List.tabulate(rsSize)(i => pending(i) := false.B)
    }
    instRdy := VecInit(List.tabulate(rsSize)(i => src1Rdy(i) && src2Rdy(i) && valid(i) && pending(i)))
    when(io.freeCheckpoint.get.valid) {
      valid(io.freeCheckpoint.get.bits) := false.B
    }
    ...
    when(io.in.fire()) {
      priorityMask(enqueueSelect) := (valid.asUInt & pending.asUInt).asBools
    }
    when(io.out.fire()) {
      (0 until rsSize).map(i => priorityMask(i)(dequeueSelect) := false.B)
    }
