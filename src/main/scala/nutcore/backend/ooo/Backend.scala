...
//    rf.write(rob.io.wb(0).rfDest, rob.io.wb(0).rfData)
//  }
...
//    when(rob.io.rvalid(2 * i) && rob.io.rcommited(2 * i)) {
//      inst(i).decode.data.src1 := rob.io.rprf(2 * i)
//    }
//    inst(i).decode.data.src2 := rf.read(rfSrc(2 * i + 1))
//    when(rob.io.rvalid(2 * i + 1) && rob.io.rcommited(2 * i + 1)) {
//      inst(i).decode.data.src2 := rob.io.rprf(2 * i + 1)
//    }
//  })
...
//  when((rob.io.empty || flushBackend) && !haveUnfinishedStore) {
//    blockReg := false.B
//  }
//  when(io.in(0).bits.ctrl.isBlocked && io.in(0).fire()) {
//    blockReg := true.B
//  }
//  val mispredictionRecoveryReg = RegInit(false.B)
//  when(io.redirect.valid && io.redirect.rtype === 1.U) {
//    mispredictionRecoveryReg := true.B
//  }
//  when(rob.io.empty || flushBackend) {
//    mispredictionRecoveryReg := false.B
//  }
...
//  // fix wen
//  when(csr.io.wenFix) {
//    csrcommit.decode.ctrl.rfWen := false.B
//  }
//
...
//    if (i != 0) {
//      notFirstMask(i) := notFirstMask(i - 1) | commitValidPriority(i - 1)
//    }
//  }
...
//  for (i <- 0 until WritebackPriority.size) {
//    if (i != 0) {
//      notSecondMask(i) := notSecondMask(i - 1) | secondCommitValid(i - 1)
//    }
//  }
