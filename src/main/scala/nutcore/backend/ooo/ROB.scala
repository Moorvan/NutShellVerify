...
class ROB(implicit val p: NutCoreConfig) extends NutCoreModule with HasInstrType with HasBackendConst with HasRegFileParameter {
  ...
  for (i <- 0 to storePageFault) {
    io.beUop.decode.cf.exceptionVec(i) := false.B
  }
  ...
  when(io.exception) {
    cancelScommit := true.B
  }
  when(io.flush) {
    cancelScommit := false.B
  }
...
//  BoringUtils.addSource(robStoreInstVec, "ROBStoreInstVec")
  ...
  Debug() {
    ...
    Debug("\n[ROB] ")
    for (i <- 0 to (robSize - 1)) {
      when(ringBufferHead === i.U) {
        Debug("h")
      }
        .elsewhen(ringBufferTail === i.U) {
          Debug("t")
        }
        .otherwise {
          Debug(" ")
    ...
    for (i <- 0 to (robSize - 1)) {
      ...
      when(valid(i)(0) || valid(i)(1)) {
        Debug("  valid")
      }
      when(ringBufferHead === i.U) {
        Debug("  head")
      }
      when(ringBufferTail === i.U) {
        Debug("  tail")
      }
      Debug("\n")
    }
